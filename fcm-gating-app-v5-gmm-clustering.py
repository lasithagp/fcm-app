...
def generate_synthetic_data_ui():
    ...
    with st.sidebar.expander("Synthetic Data Generation Settings", expanded=True):
        method = st.selectbox("Select Method", ["GMM", "Multivariate Normal"],
                              index=0,
                              help="Select the method for generating synthetic data. \
                              \nGMM uses Gaussian Mixture Models, while Multivariate Normal uses multivariate normal distribution. \
                                  \nThe GMM method is recommended for multimodal populations. \
                                      \nThe Multivariate Normal method is recommended for unimodal populations."
                                  )
...
def evaluate_synthetic_data(original_data, synthetic_data, sample_size=10000, population_name=None):
    ...
    try:
        ...
        # Mutual Information (replaced with regression-based method for stability)
        try:
            mi_scores = []
            for i in range(original_sample.shape[1]):
        ...
        try:
            ...
            for i in range(original_sample.shape[1]):
                wasserstein_scores.append(wasserstein_distance(original_sample[:, i], synthetic_sample[:, i]) / 
                                          (np.std(original_sample[:, i]) + np.std(synthetic_sample[:, i])))             # Normalize by std deviation wd = wasserstein_distance(x, y) / (np.std(x) + np.std(y))
...
def plot_dimensionality_reduction(original, synthetic, population, method='PCA'):
    """Interactive dimensionality reduction comparison plot."""
    import plotly.express as px
    import pandas as pd
    ...
    labels = ['Original'] * len(original) + ['Synthetic'] * len(synthetic)
    ...
    df = pd.DataFrame({
        ...
        'y': reduced_data[:, 1],
        'Type': labels,
        'Dataset': ['Original'] * len(original) + ['Synthetic'] * len(synthetic)
    })
    
    # Create interactive plot
    fig = px.scatter(df, x='x', y='y', color='Dataset', 
                    symbol='Type', opacity=0.6,
                    color_discrete_sequence=px.colors.qualitative.Set1,                     
                    title=f"{method} Comparison: {population}",
...
def data_quality_metrics_ui():
    ...
    with st.sidebar.expander("📊 Data Quality Evaluation Settings", expanded=True):
        ...
        dr_method = st.selectbox(
            ...
            index=2,
            help="Select the method for dimensionality reduction. \
                \n PCA is faster but may not capture complex structures. \
                \n t-SNE and UMAP are slower but can capture more complex structures.",                    
            key='dr_method'
        )
        ...
        max_features = st.number_input(
            ...
            max_value=10, 
            value=4,
            help="Maximum number of features to visualize per row in the distribution comparison plot. \
                \n This will help in organizing the plots and making them more readable.",
        ...
        sample_size = st.number_input(
            ...
            max_value=50000, 
            value=10000,
            help="Sample size for evaluation metrics. \
                \n Default value will be used if the sample size is larger than the data size. \
                    \n This will be used for silhouette score, mutual information, and Wasserstein distance calculations. \
                \n A larger sample size may improve accuracy but will take longer to compute.",
    ...
    if st.session_state.evaluation_results:
        ...
        for i, result in enumerate(st.session_state.evaluation_results):
            with tabs[i]:
                ...
                try:
                    # Get data
                    original = np.concatenate([d.values for d in st.session_state.gated_data.values()])
